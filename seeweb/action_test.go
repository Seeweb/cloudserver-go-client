...
func TestActionGet(t *testing.T) {
	...
	want := &SeewebActionGetResponse{
		...
		Action: &Action{
			...
			Status:       "completed",
			User:         "admin",
			CreatedAt:    time.Date(2019, time.April, 30, 16, 33, 3, 317800000, time.FixedZone("UTC", 0)),
			StartedAt:    time.Date(2019, time.April, 30, 16, 33, 3, 317019000, time.FixedZone("UTC", 0)),
			CompletedAt:  time.Date(2019, time.April, 30, 16, 34, 4, 159922000, time.FixedZone("UTC", 0)),
			Resource:     "ec200016",
			ResourceType: "ECS",
			Type:         "delete_server",
			Progress:     100,
